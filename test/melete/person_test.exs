defmodule Melete.PersonTest do
  use Melete.DataCase

  alias Melete.Person

  import Melete.PersonFixtures
  alias Melete.Person.{Account, AccountToken}

  describe "get_account_by_email/1" do
    test "does not return the account if the email does not exist" do
      refute Person.get_account_by_email("unknown@example.com")
    end

    test "returns the account if the email exists" do
      %{id: id} = account = account_fixture()
      assert %Account{id: ^id} = Person.get_account_by_email(account.email)
    end
  end

  describe "get_account_by_email_and_password/2" do
    test "does not return the account if the email does not exist" do
      refute Person.get_account_by_email_and_password("unknown@example.com", "hello world!")
    end

    test "does not return the account if the password is not valid" do
      account = account_fixture()
      refute Person.get_account_by_email_and_password(account.email, "invalid")
    end
    ...
    test "returns the account if the email and password are valid" do
      ...
      assert %Account{id: ^id} =
               Person.get_account_by_email_and_password(account.email, valid_account_password())
    end
  end
  ...
  describe "get_account!/1" do
    test "raises if id is invalid" do
      assert_raise Ecto.NoResultsError, fn ->
        Person.get_account!(-1)
      end
    end

    test "returns the account with the given id" do
      %{id: id} = account = account_fixture()
      assert %Account{id: ^id} = Person.get_account!(account.id)
    end
  end

  describe "register_account/1" do
    test "requires email and password to be set" do
      {:error, changeset} = Person.register_account(%{})
    ...
    test "validates email and password when given" do
      {:error, changeset} = Person.register_account(%{email: "not valid", password: "not valid"})
    ...
    test "validates maximum values for email and password for security" do
      ...
      {:error, changeset} = Person.register_account(%{email: too_long, password: too_long})
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = Person.register_account(%{email: email})
      assert "has already been taken" in errors_on(changeset).email

      # Now try with the upper cased email too, to check that email case is ignored.
      {:error, changeset} = Person.register_account(%{email: String.upcase(email)})
      assert "has already been taken" in errors_on(changeset).email
    end

    test "registers accounts with a hashed password" do
      email = unique_account_email()
      {:ok, account} = Person.register_account(valid_account_attributes(email: email))
  ...
  describe "change_account_registration/2" do
    test "returns a changeset" do
      assert %Ecto.Changeset{} = changeset = Person.change_account_registration(%Account{})
    ...
    test "allows fields to be set" do
      ...
      changeset =
        Person.change_account_registration(
          %Account{},
  ...
  describe "change_account_email/2" do
    test "returns a account changeset" do
      assert %Ecto.Changeset{} = changeset = Person.change_account_email(%Account{})
  ...
  describe "apply_account_email/3" do
    ...
    test "requires email to change", %{account: account} do
      {:error, changeset} = Person.apply_account_email(account, valid_account_password(), %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{account: account} do
      {:error, changeset} =
        Person.apply_account_email(account, valid_account_password(), %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{account: account} do
      ...
      {:error, changeset} =
        Person.apply_account_email(account, valid_account_password(), %{email: too_long})
    ...
    test "validates email uniqueness", %{account: account} do
      ...
      {:error, changeset} = Person.apply_account_email(account, password, %{email: email})
    ...
    test "validates current password", %{account: account} do
      {:error, changeset} =
        Person.apply_account_email(account, "invalid", %{email: unique_account_email()})
    ...
    test "applies the email without persisting it", %{account: account} do
      email = unique_account_email()
      {:ok, account} = Person.apply_account_email(account, valid_account_password(), %{email: email})
      assert account.email == email
      assert Person.get_account!(account.id).email != email
    end
  end
    ...
    test "sends token through notification", %{account: account} do
      token =
        extract_account_token(fn url ->
          Person.deliver_account_update_email_instructions(account, "current@example.com", url)
        end)
  ...
  describe "update_account_email/2" do
    setup do
      ...
      token =
        extract_account_token(fn url ->
          Person.deliver_account_update_email_instructions(%{account | email: email}, account.email, url)
        end)
    ...
    test "updates the email with a valid token", %{account: account, token: token, email: email} do
      assert Person.update_account_email(account, token) == :ok
    ...
    test "does not update email with invalid token", %{account: account} do
      assert Person.update_account_email(account, "oops") == :error
    ...
    test "does not update email if account email changed", %{account: account, token: token} do
      assert Person.update_account_email(%{account | email: "current@example.com"}, token) == :error
      assert Repo.get!(Account, account.id).email == account.email
      assert Repo.get_by(AccountToken, account_id: account.id)
    end
    ...
    test "does not update email if token expired", %{account: account, token: token} do
      ...
      assert Person.update_account_email(account, token) == :error
      assert Repo.get!(Account, account.id).email == account.email
      assert Repo.get_by(AccountToken, account_id: account.id)
    end
  end
  ...
  describe "change_account_password/2" do
    test "returns a account changeset" do
      assert %Ecto.Changeset{} = changeset = Person.change_account_password(%Account{})
      assert changeset.required == [:password]
    end

    test "allows fields to be set" do
      changeset =
        Person.change_account_password(%Account{}, %{
  ...
  describe "update_account_password/3" do
    ...
    test "validates password", %{account: account} do
      {:error, changeset} =
        Person.update_account_password(account, valid_account_password(), %{
          password: "not valid",
    ...
    test "validates maximum values for password for security", %{account: account} do
      ...
      {:error, changeset} =
        Person.update_account_password(account, valid_account_password(), %{password: too_long})
    ...
    test "validates current password", %{account: account} do
      {:error, changeset} =
        Person.update_account_password(account, "invalid", %{password: valid_account_password()})
    ...
    test "updates the password", %{account: account} do
      ...
      assert is_nil(account.password)
      assert Person.get_account_by_email_and_password(account.email, "new valid password")
    end

    test "deletes all tokens for the given account", %{account: account} do
      _ = Person.generate_account_session_token(account)
  ...
  describe "generate_account_session_token/1" do
    ...
    test "generates a token", %{account: account} do
      token = Person.generate_account_session_token(account)
  ...
  describe "get_account_by_session_token/1" do
    ...
    test "returns account by token", %{account: account, token: token} do
      assert session_account = Person.get_account_by_session_token(token)
      assert session_account.id == account.id
    end

    test "does not return account for invalid token" do
      refute Person.get_account_by_session_token("oops")
    end

    test "does not return account for expired token", %{token: token} do
      {1, nil} = Repo.update_all(AccountToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute Person.get_account_by_session_token(token)
    end
  end
  ...
  describe "delete_account_session_token/1" do
    test "deletes the token" do
      account = account_fixture()
      token = Person.generate_account_session_token(account)
      assert Person.delete_account_session_token(token) == :ok
      refute Person.get_account_by_session_token(token)
    end
  end
  ...
  describe "deliver_account_confirmation_instructions/2" do
    ...
    test "sends token through notification", %{account: account} do
      token =
        extract_account_token(fn url ->
          Person.deliver_account_confirmation_instructions(account, url)
        end)
  ...
  describe "confirm_account/1" do
    ...
    test "confirms the email with a valid token", %{account: account, token: token} do
      assert {:ok, confirmed_account} = Person.confirm_account(token)
    ...
    test "does not confirm with invalid token", %{account: account} do
      assert Person.confirm_account("oops") == :error
    ...
    test "does not confirm email if token expired", %{account: account, token: token} do
      {1, nil} = Repo.update_all(AccountToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      assert Person.confirm_account(token) == :error
      refute Repo.get!(Account, account.id).confirmed_at
      assert Repo.get_by(AccountToken, account_id: account.id)
    end
  end
  ...
  describe "deliver_account_reset_password_instructions/2" do
    ...
    test "sends token through notification", %{account: account} do
      token =
        extract_account_token(fn url ->
          Person.deliver_account_reset_password_instructions(account, url)
        end)
  ...
  describe "reset_account_password/2" do
    ...
    test "validates password", %{account: account} do
      {:error, changeset} =
        Person.reset_account_password(account, %{
          password: "not valid",
          password_confirmation: "another"
        })
    ...
    test "validates maximum values for password for security", %{account: account} do
      too_long = String.duplicate("db", 100)
      {:error, changeset} = Person.reset_account_password(account, %{password: too_long})
      assert "should be at most 72 character(s)" in errors_on(changeset).password
    end

    test "updates the password", %{account: account} do
      {:ok, updated_account} = Person.reset_account_password(account, %{password: "new valid password"})
    ...
    test "deletes all tokens for the given account", %{account: account} do
      _ = Person.generate_account_session_token(account)
      {:ok, _} = Person.reset_account_password(account, %{password: "new valid password"})
      refute Repo.get_by(AccountToken, account_id: account.id)
    end
  end
