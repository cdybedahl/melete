defmodule MeleteWeb.AccountAuthTest do
  use MeleteWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Melete.Person
  alias MeleteWeb.AccountAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, MeleteWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_account/3" do
    test "stores the account token in the session", %{conn: conn, account: account} do
      ...
      assert Person.get_account_by_session_token(token)
    end
  ...
  describe "logout_account/1" do
    test "erases session and cookies", %{conn: conn, account: account} do
      account_token = Person.generate_account_session_token(account)
      ...
      assert redirected_to(conn) == ~p"/"
      refute Person.get_account_by_session_token(account_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "accounts_sessions:abcdef-token"
      MeleteWeb.Endpoint.subscribe(live_socket_id)
