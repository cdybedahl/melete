defmodule MeleteWeb.AccountResetPasswordLive do
  ...
  def handle_event("reset_password", %{"account" => account_params}, socket) do
    case Person.reset_account_password(socket.assigns.account, account_params) do
      {:ok, _} ->
        {:noreply,
         socket
  ...
  def handle_event("validate", %{"account" => account_params}, socket) do
    changeset = Person.change_account_password(socket.assigns.account, account_params)
    {:noreply, assign_form(socket, Map.put(changeset, :action, :validate))}
  end

  defp assign_account_and_token(socket, %{"token" => token}) do
    if account = Person.get_account_by_reset_password_token(token) do
