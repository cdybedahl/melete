defmodule MeleteWeb.AccountAuth do
  use MeleteWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_melete_web_account_remember_me"
  ...
  def log_out_account(conn) do
    ...
    account_token && Person.delete_account_session_token(account_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      MeleteWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_account(conn, _opts) do
    ...
    account = account_token && Person.get_account_by_session_token(account_token)
  ...
  the current_account:
      ...
      defmodule MeleteWeb.PageLive do
        use MeleteWeb, :live_view

        on_mount {MeleteWeb.AccountAuth, :mount_current_account}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{MeleteWeb.AccountAuth, :ensure_authenticated}] do
  ...
  defp mount_current_account(socket, session) do
    Phoenix.Component.assign_new(socket, :current_account, fn ->
      if account_token = session["account_token"] do
        Person.get_account_by_session_token(account_token)
      end
    end)
  end
